...
module.exports = async (client, m) => {
    ...
    // --- Detectar usuarios que entran o salen del grupo ---
    if (m.isGroup && m.message.groupParticipant) {
        const action = Object.keys(m.message.groupParticipant)[0]; // 'add' o 'remove'
        const participants = m.message.groupParticipant[action];
        require('./commands/_welcome.js')(client, m, action, participants);
    }
    ...
    if (global.comandos.has(command)) {
        ...
        try {
            ...
            if (cmdData.isAdmin && !isAdmins) return m.reply("Debes ser admin para usar este comando");
            if (cmdData.isBotAdmin && !isBotAdmins) return m.reply("El bot debe ser admin para ejecutar esto");
            if (cmdData.isPrivate && m.isGroup) return m.reply("Este comando solo se puede usar en privado");
