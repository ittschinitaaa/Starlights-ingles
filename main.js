...
const welcome = require('./commands/_welcome.js'); // Plugin de bienvenida y despedida
...
module.exports = async (client, m) => {
  ...
  // --- Bloque de bienvenida / despedida ---
  if (m.isGroup && m.message?.groupParticipantUpdate) {
    const update = m.message.groupParticipantUpdate;

    if (update.added) {
      welcome(client, m, 'add', update.added);
    }
    if (update.removed) {
      welcome(client, m, 'remove', update.removed);
    }
  }
